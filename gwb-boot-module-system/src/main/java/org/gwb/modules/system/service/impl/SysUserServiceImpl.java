package org.gwb.modules.system.service.impl;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public Set<String> getUserPermissionsSet(String username) {
        ...
        log.info("-------通过数据库读取用户拥有的权限Perms------username： " + username + ",Perms size: "
            ...
            for (SysUserDepart depart : userDepartList) {
                ...
                if (!Arrays.asList(arr).contains(depart.getDepId())) {
                    List<SysDepartRole> sysDepartRoleList = sysDepartRoleMapper
                            .selectList(new QueryWrapper<SysDepartRole>().lambda().eq(SysDepartRole::getDepartId,
                                    depart.getDepId()));
     ...
     */
    @Override
    public SysUser gerUserInfo(String username) {
    ...
    // user.setUserIdentity(sysUser.getUserIdentity());
